...
import { FlarebondMintableERC20 } from './FlarebondMintableERC20.sol';
import { IFlarebondMintableERC20, ILegacyMintableERC20 } from './IFlarebondMintableERC20.sol';
...
contract FlarebondBridge is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    using SafeERC20 for IERC20;

    uint32 internal constant RECEIVE_DEFAULT_GAS_LIMIT = 200_000;

    IFlareRelayer public  MESSENGER;
    FlarebondBridge public  OTHER_BRIDGE;
    ...
    IFlarebondMintableERC20 ethContract;
    ...
    function initialize(address initialOwner, address payable _messenger, address payable _otherBridge) initializer public {
        ...
        OTHER_BRIDGE = FlarebondBridge(_otherBridge);
    ...
    modifier onlyEOA() {
        require(!Address.isContract(msg.sender), "FlarebondBridge: function can only be called from an EOA");
        _;
    }
    ...
    function setEthERC20ContractAddress(address _ethAddress) public onlyOwner {
        ethContract = IFlarebondMintableERC20(_ethAddress);
    }
...
function bridgeETH(uint32 _minGasLimit, bytes calldata _extraData) public payable onlyEOA {
    ...
    function finalizeBridgeETH(
        ...
        // onlyOtherBridge
    {
        // require(paused() == false, "FlarebondBridge: paused");
        require(msg.value == _amount, "FlarebondBridge: amount sent does not match amount required");
        require(_to != address(this), "FlarebondBridge: cannot send to self");
        require(_to != address(MESSENGER), "FlarebondBridge: cannot send to messenger");
        _emitETHBridgeFinalized(_from, _to, _amount, _extraData);

        bool success = call(_to, gasleft(), _amount, hex"");
        require(success, "FlarebondBridge: ETH transfer failed");
    }
    ...
    {
        // require(paused() == false, "FlarebondBridge: paused");
        if (_isFlarebondMintableERC20(_localToken)) {
            require(
                _isCorrectTokenPair(_localToken, _remoteToken),
                "FlarebondBridge: wrong remote token for Flarebond Mintable ERC20 local token"
            );

            FlarebondMintableERC20(_localToken).mint(_to, _amount);
        } else {
    ...
    function _initiateBridgeETH(
        ...
        internal
    {
        require(msg.value == _amount, "FlarebondBridge: bridging ETH must include sufficient ETH value");
        ...
        internal
    {
        if (_isFlarebondMintableERC20(_localToken)) {
            ...
            FlarebondMintableERC20(_localToken).burn(_from, _amount);
        } else {
    ...
    function _isFlarebondMintableERC20(address _token) internal view returns (bool) {
        return ERC165Checker.supportsInterface(_token, type(ILegacyMintableERC20).interfaceId) ||
            ERC165Checker.supportsInterface(_token, type(IFlarebondMintableERC20).interfaceId);
    }
    ...
    function _isCorrectTokenPair(address _mintableToken, address _otherToken) internal view returns (bool) {
        if (ERC165Checker.supportsInterface(_mintableToken, type(ILegacyMintableERC20).interfaceId)) {
            ...
            return _otherToken == IFlarebondMintableERC20(_mintableToken).remoteToken();
        }
    }
